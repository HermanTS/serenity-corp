...
{
    GpImage *image = static_cast<GpImage*>(img.handle.ptr);
...
nk_gdip_image_free(struct nk_image image)
{
    if (!image.handle.ptr)
        return;
    GdipDisposeImage(static_cast<GpImage*>(image.handle.ptr));
}
...
nk_gdipfont_create_from_collection(int size){
    ...
    INT count;
    GdipFont *font = (GdipFont*)calloc(1, sizeof(GdipFont));
    if( GdipGetFontCollectionFamilyCount(gdip.fontCollection[gdip.curFontCollection], &count) ) return NULL;
    families = (GpFontFamily**)calloc(1, sizeof(GpFontFamily*));
    if( !families ) return NULL;
    if( GdipGetFontCollectionFamilyList(gdip.fontCollection[gdip.curFontCollection], count, families, &count) ) return NULL;
...
{
    if( !gdip.fontCollection[gdip.curFontCollection] )
        if( GdipNewPrivateFontCollection(&gdip.fontCollection[gdip.curFontCollection]) ) return NULL;
    if( GdipPrivateAddMemoryFont(gdip.fontCollection[gdip.curFontCollection], membuf, membufSize) ) return NULL;
...
nk_gdipfont_create_from_file(const WCHAR* filename, int size)
{
    if( !gdip.fontCollection[gdip.curFontCollection] )
        if( GdipNewPrivateFontCollection(&gdip.fontCollection[gdip.curFontCollection]) ) return NULL;
    if( GdipPrivateAddFontFile(gdip.fontCollection[gdip.curFontCollection], filename) ) return NULL;    
    return nk_gdipfont_create_from_collection(size);
}
